...
<body>
    <div class="body-wrap">
        <div class="menu-wrap">
            ...
            <div class="mini-menu">
                <div class="now-mini_menu">고객사 관리</div> <!-- 현재 위치의 메뉴 -->  <!-- 1. 변경 메뉴에 맞게 내용 변경하기 -->
            </div>
        </div> <!-- 상단 메뉴바 wrap -->
        <div class="content-wrap"> 
            <div class="contentTitle_container">
                <div class="contentTitle-wrap">
                    <h1>고객사 목록</h1> <!-- 2. 변경 메뉴에 맞게 내용 변경 하기 -->
                </div>
                <div class="search-wrap">
                    <form name="searchForm" method="post">
                        <div class="search_container"> 
                            <select name="searchOption" class="searchOption"> <!-- 해당 메뉴에 해당하는 옵션으로 변경 -->
                                <option value="all">전체</option>
                                <option value="CUSTOMER_COMPANY">고객사</option>
                                <option value="CUSTOMER_MANAGER">담당자</option>
                                <option value="CUSTOMER_MAN_TEL">담당자 연락처</option>
                                <option value="CUSTOMER_INSERT_DATE">등록일</option>
                            </select>
                ...
                <div class="mainContent-wrap">
                    <table id="mainTable">
                        <thead>
                            <tr> <!-- 4. tr 째로 복사하여 붙여넣기 -->
                                ...
                                <th style="width: 302px;">
                                    <div class="th_div">담당자 연락처</div>
                                </th>
                                <th style="width: 306px;">
                                    <div class="th_div">등록일</div>
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                        <c:forEach var="row" items="${map.list}">
                            <tr> <!-- 5. tr 째로 붙여넣기 -->
                                <td>
                                	<div class="td_div"><a href="/view_customer.do?BNO=${row.BNO}"></a></div>
                            	</td>
                                <td>
                                    <div class="td_div"><a href="/view_customer.do?BNO=${row.BNO}">${row.CUSTOMER_COMPANY}</a></div>
                                </td>
                                <td>
                                    <div class="td_div"><a href="/view_customer.do?BNO=${row.BNO}">${row.CUSTOMER_MANAGER}</a></div>
                                </td>
                                <td>
                                    <div class="td_div"><a href="/view_customer.do?BNO=${row.BNO}">${row.CUSTOMER_MAN_TEL}</a></div>
                                </td>
                                <td>
                                    <div class="td_div"><a href="/view_customer.do?BNO=${row.BNO}">${row.CUSTOMER_INSERT_DATE}</a></div>
                                </td>
                            </tr>
                        </c:forEach>
                        </tbody>
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            $('#insertButton').on('click', function(){
                location.href='/go_insert_customer.do'; // 6. 등록으로 이동하는 url 붙여넣기
            });
